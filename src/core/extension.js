...
// A base class marks CamelCase alias support by its name: inheriting from it
// declares "every snake_case attribute on this type is also readable in
// camelCase", which is what a generic converter base actually provides. The
// marker used to be the hardcoded substring "camelcase"; it is now a setting
// whose default is that same substring, so out-of-the-box behaviour is
// unchanged while a project whose converter base is called something else can
// say so instead of finding the feature silently inert.
// The decorator that maps ASCII argument spellings onto diacritic ones, and the
// module-level list of keys it leaves alone, are named by convention rather
// than by any standard — so both names are settings. The defaults are the
// spellings this extension has always looked for, which keeps existing
// workspaces working untouched while a project that names them differently
// can say so instead of finding the feature silently inert.
const DEFAULT_UMLAUT_DECORATOR_NAMES = ["convert_umlaut_kwargs"];
const DEFAULT_UMLAUT_EXCLUDE_SYMBOLS = ["_exclude_umlaut_kwargs"];
let umlautNamesCache = null;

function escapeForRegExp(value) {
  return String(value || "").replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
}

function getUmlautNames() {
  if (umlautNamesCache) {
    return umlautNamesCache;
  }
  const read = (key, fallback) => {
    try {
      const value = getConfig().get(key, fallback);
      ...
      // No extension host (unit tests, tooling): the default stands.
    }
    return fallback;
  };
  const decorators = read("umlautDecoratorNames", DEFAULT_UMLAUT_DECORATOR_NAMES);
  const excludes = read("umlautExcludeSymbols", DEFAULT_UMLAUT_EXCLUDE_SYMBOLS);
  const dec = decorators.map(escapeForRegExp).join("|");
  const exc = excludes.map(escapeForRegExp).join("|");
  umlautNamesCache = {
    decorators,
    excludes,
    isDecorator: new RegExp("^@(?:" + dec + ")\\b"),
    decoratorCall: new RegExp("^@(?:" + dec + ")(?:\\s*\\(([\\s\\S]*)\\))?\\s*$"),
    aliasPlusLiteral: new RegExp("^(?:" + exc + ")\\s*\\+\\s*([\\[(][\\s\\S]*[\\])])$"),
    excludeAssignment: new RegExp("(?:" + exc + ")\\s*=\\s*\\[")
  };
  return umlautNamesCache;
}

function resetUmlautNamesCache() {
  umlautNamesCache = null;
}

const DEFAULT_CAMEL_CASE_ALIAS_BASE_NAMES = ["camelcase"];
let camelCaseAliasBaseNamesCache = null;

function getCamelCaseAliasBaseNames() {
  if (camelCaseAliasBaseNamesCache) {
    return camelCaseAliasBaseNamesCache;
  }
  let configured = DEFAULT_CAMEL_CASE_ALIAS_BASE_NAMES;
  try {
    const value = getConfig().get("camelCaseAliasBaseNames", DEFAULT_CAMEL_CASE_ALIAS_BASE_NAMES);
    if (Array.isArray(value)) {
      configured = value;
    }
  } catch (error) {
  ...
  camelCaseAliasBaseNamesCache = configured
    .map((name) => normalizeComparableToken(name))
    .filter((name) => name.length > 0);
  return camelCaseAliasBaseNamesCache;
}

function resetCamelCaseAliasBaseNamesCache() {
  camelCaseAliasBaseNamesCache = null;
}

function isCamelCaseAliasBaseName(typeName) {
  const normalized = normalizeComparableToken(typeName);
  if (!normalized) {
    return false;
  }
  return getCamelCaseAliasBaseNames().some((marker) => normalized.includes(marker));
}
function isIgnoredPythonTypeToken(typeName) {
  const normalized = String(typeName || "").toLowerCase();
  return PYTHON_IGNORED_TYPE_TOKENS.has(normalized) || isCamelCaseAliasBaseName(typeName);
}
...
// Optional preferred module path suffix, used only to break ties when several workspace
// files declare exclusion defaults. Empty means "no preference": the first
// discovered config wins. Configurable via `robotCompanion.umlautDecoratorModuleSuffix`.
const DEFAULT_CONVERT_UMLAUT_DECORATION_FILE_SUFFIX = "";
// Used only when no exclusion defaults are discovered in the workspace.
// Projects declare their own exclusions in the decorator module itself.
...
function activate(context) {
  ...
  context.subscriptions.push(
    ...
    vscode.workspace.onDidChangeConfiguration((event) => {
      ...
      if (
        event.affectsConfiguration(`${EXT_CONFIG_ROOT}.umlautDecoratorNames`) ||
        event.affectsConfiguration(`${EXT_CONFIG_ROOT}.umlautExcludeSymbols`)
      ) {
        resetUmlautNamesCache();
        enumHintService.invalidateAll();
        returnComputeWorker.invalidateAll();
      }
      if (event.affectsConfiguration(`${EXT_CONFIG_ROOT}.camelCaseAliasBaseNames`)) {
        // read while parsing class declarations, so the index itself is stale
    ...
    vscode.workspace.onDidCreateFiles((event) => {
      ...
      if (pythonFiles.length > 0) {
        ...
        // pythonFiles holds vscode.Uri values, which have no `.uri` property,
        // so the targeted branch that used to sit here never ran and every
        // create/delete already fell through to a full invalidation. Dropping
        // it changes no behaviour and stops the code claiming a precision it
        // never had. A full invalidation is also the honest choice for these
        // two events: adding or removing a module can change how types in
        // *other* files resolve, which a per-file eviction would miss. Saves
        // take the fine-grained path (invalidateTypePreviewByFileUris), where
        // the set of affected files really is known.
        returnComputeWorker.invalidateAll();
        runtimeCacheService.invalidateAll();
      }
    }),
...
function stripRobotCellSeparator(textAfterPrefix) {
  ...
  // Robot tables commonly use 4 spaces as cell separator.
...
function parsePythonClassHeader(lines, startIndex) {
  ...
  const supportsCamelCaseAccess = rawBaseTypeNames.some(
    (baseTypeName) => isCamelCaseAliasBaseName(baseTypeName)
  );
  const baseTypeNames = uniqueStrings(
    rawBaseTypeNames.filter(
      (baseTypeName) =>
        !isIgnoredPythonTypeToken(baseTypeName) &&
...
function parseConvertUmlautDecoratorMetadataFromExpression(expression) {
  ...
  if (!getUmlautNames().isDecorator.test(trimmedExpression)) {
    return {
      applied: false,
      extraExcludeKeys: [],
      unsupportedExcludeExpression: ""
  ...
  const callMatch = trimmedExpression.match(getUmlautNames().decoratorCall);
...
function parseConvertUmlautDecoratorExcludeExpression(expressionText) {
  ...
  if (!expression || getUmlautNames().excludes.includes(expression)) {
    return {
      extraExcludeKeys: [],
      unsupportedExcludeExpression: ""
    };
  ...
  const aliasPlusLiteralMatch = expression.match(getUmlautNames().aliasPlusLiteral);
...
function parseConvertUmlautDecoratorConfigFromPythonSource(source, sourceFilePath = "") {
  ...
  const { decorators, excludes, excludeAssignment } = getUmlautNames();
  if (!excludes.some((e) => sourceText.includes(e)) ||
      !decorators.some((d) => sourceText.includes("def " + d))) {
    return undefined;
  }

  const match = sourceText.match(excludeAssignment);
  if (!match) {
    return undefined;
  }
...
function resolveWorkspaceConvertUmlautDecoratorConfig(contributions, preferredModuleSuffix) {
  const suffix =
    typeof preferredModuleSuffix === "string"
      ? preferredModuleSuffix
      : getConvertUmlautDecoratorModuleSuffix();
  ...
  for (const contribution of contributions || []) {
    ...
    if (suffix && normalizedSourcePath.endsWith(suffix)) {
...
function finalizePythonKeywordDefinitionForIndex(keywordDefinition, options = {}) {
  ...
  if (shouldConvert && convertMetadata?.unsupportedExcludeExpression) {
    logRobotCompanionTrace(
      "Unsupported umlaut-decorator exclude expression; falling back to default exclusions.",
      {
...
function extractComparableTypeNamesFromAnnotation(annotation) {
  ...
  for (const token of tokens) {
    ...
    if (!normalizedToken || isIgnoredPythonTypeToken(normalizedToken)) {
      continue;
    }
...
function prepareDocumentationColorMarkupForRender(markdown) {
  ...
  const protectIntentionalLineBreaks = (source) =>
    String(source || "").replace(/<br\s*\/?>/gi, () => {
      const token = `@@RMC_DOC_LINE_BREAK_${lineBreakCounter}@@`;
      lineBreakCounter += 1;
      replacements.set(token, "<br>");
      return token;
    });
  ...
  const transformInline = (line) => {
    let transformed = protectIntentionalLineBreaks(line);
...
function normalizeDocumentationParagraphLines(markdownLines) {
  const normalizedLines = [];
  let proseLine = "";
  let inFence = false;

  const flushProseLine = () => {
    if (!proseLine) {
      return;
    }
    normalizedLines.push(proseLine);
    proseLine = "";
  };

  for (const rawLine of Array.isArray(markdownLines) ? markdownLines : []) {
    ...
    proseLine = proseLine ? `${proseLine.trimEnd()} ${line.trim()}` : line.trimEnd();
  }
...
function isInlineDocumentationItemContinuation(entry, itemNestingLevel) {
  const text = String(entry?.text || "");
  const entryNestingLevel = Math.max(0, Number(entry?.nestingLevel) || 0);
  if (entryNestingLevel !== itemNestingLevel || text.trim().length === 0) {
    return false;
  }

  const nestingPrefix = "  ".repeat(entryNestingLevel);
  const textAfterNesting = text.startsWith(nestingPrefix) ? text.slice(nestingPrefix.length) : text;
  return /^[ \t]+\S/.test(textAfterNesting);
}
...
function buildInlineDocumentationRenderItems(documentUri, lineEntries, fragmentStartLine) {
  ...
  let listItemNestingLevel = 0;
  let arrowLine = "";
  let arrowStartLine = fragmentStartLine;
  let arrowNestingLevel = 0;
  ...
  const flushParagraph = () => {
    ...
    items.push(
      createDocumentationRenderItem("chunk", normalizeDocumentationParagraphLines(paragraphLines), {
  ...
  const flushListItem = () => {
    ...
    items.push(
      createDocumentationRenderItem("list-item", [listItemLine], {
  ...
  const flushArrowLine = () => {
    if (!arrowLine) {
      return;
    }
    items.push(
      createDocumentationRenderItem("arrow-line", [arrowLine], {
        commandUri: buildOpenLocationCommandUri(documentUri, arrowStartLine),
        label: `Open arrow line ${arrowStartLine + 1}`
      })
    );
    arrowLine = "";
  };
  ...
  for (const entry of lineEntries) {
    ...
    if (isListItem) {
      ...
      listItemNestingLevel = Math.max(0, Number(entry?.nestingLevel) || 0);
      continue;
    }
    ...
    if (isArrowLine) {
      ...
      flushArrowLine();
      arrowLine = text;
      arrowStartLine = sourceLine;
      arrowNestingLevel = Math.max(0, Number(entry?.nestingLevel) || 0);
      continue;
    }

    if (arrowLine && isInlineDocumentationItemContinuation(entry, arrowNestingLevel)) {
      arrowLine = `${arrowLine.trimEnd()} ${text.trim()}`;
      continue;
    }

    flushArrowLine();

    if (listItemLine && isInlineDocumentationItemContinuation(entry, listItemNestingLevel)) {
      listItemLine = `${listItemLine.trimEnd()} ${text.trim()}`;
      continue;
    }
...
function buildStructuredDocumentationRenderItems(documentUri, lineEntries, fragmentStartLine) {
  ...
  for (const entry of lineEntries) {
    ...
    if (listItemLines.length > 0) {
      if (!isArrowLine && /^[ \t]+\S/.test(text)) {
        const previousLineIndex = listItemLines.length - 1;
        listItemLines[previousLineIndex] = `${listItemLines[previousLineIndex].trimEnd()} ${text.trim()}`;
        continue;
      }
...
function getConvertUmlautDecoratorModuleSuffix() {
  const raw = String(
    getConfig().get("umlautDecoratorModuleSuffix", DEFAULT_CONVERT_UMLAUT_DECORATION_FILE_SUFFIX) ||
      DEFAULT_CONVERT_UMLAUT_DECORATION_FILE_SUFFIX
  ).trim();
...
module.exports = {
  ...
  __test__: {
    ...
    normalizeDocumentationParagraphLines,

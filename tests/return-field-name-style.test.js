...
// settings the extension reads can be overridden per-assertion; empty means
// every read falls through to the shipped default.
        ...
        workspace: {
          getConfiguration: () => ({
            get: (key, fallbackValue) =>
              Object.prototype.hasOwnProperty.call(testConfigOverrides, key)
                ? testConfigOverrides[key]
                : fallbackValue
          }),
...
class CamelDirect(CamelCaseModel):
    status_code: int
...
function runReturnFieldNameStyleTests() {
  const index = createIndex([
    ...
    },
    {
      ...
      fields: [{ name: "sub_order", annotation: "StatusPayload" }]
    },
    {
      name: "CamelResponse",
    ...
    },
    {
      ...
      supportsCamelCaseAccess: true,
      fields: [{ name: "order_key", annotation: "str" }]
    },
    {
      name: "PlainChild",
  ...
  assert(camelOnly.secondLevel.includes("${resp.statusCode.orderKey}"));
  ...
  assert(snakeOnly.secondLevel.includes("${resp.status_code.order_key}"));
  ...
  assert(both.secondLevel.includes("${resp.statusCode.orderKey}"));
  assert(both.secondLevel.includes("${resp.status_code.order_key}"));
  assert(both.secondLevel.includes("${resp.plainChild.raw_field}"));
  assert(both.secondLevel.includes("${resp.plain_child.raw_field}"));
  assert(!both.secondLevel.includes("${resp.statusCode.order_key}"));
  assert(!both.secondLevel.includes("${resp.status_code.orderKey}"));
  ...
  assert(inheritedCamelOnly.firstLevel.includes("${wrapped.subOrder}"));
  assert(!inheritedCamelOnly.firstLevel.includes("${wrapped.sub_order}"));
  assert(inheritedCamelOnly.secondLevel.includes("${wrapped.subOrder.orderKey}"));
}
...
function runPropertyInclusionTests() {
  const index = createIndex([
    {
      ...
      fields: [{ name: "status_code", annotation: "str" }],
      properties: [
        { name: "order_key", annotation: "str" },
        { name: "orderKey", annotation: "str" }
      ]
    }
  ]);
  ...
  assert.deepStrictEqual(camelOnly.firstLevel, ["${payload.statusCode}", "${payload.orderKey}"]);
  ...
  assert(snakeOnly.firstLevel.includes("${payload.order_key}"));
  assert(snakeOnly.firstLevel.includes("${payload.orderKey}"));
  ...
  assert(technicalWithProperties.some((line) => line.includes(".order_key")));
  assert(technicalWithProperties.some((line) => line.includes(".orderKey")));
  ...
  assert(!technicalWithoutProperties.some((line) => line.includes(".order_key")));
  assert(!technicalWithoutProperties.some((line) => line.includes(".orderKey")));
  ...
  const aliasIndex = createIndex([
    {
      ...
      supportsCamelCaseAccess: false,
      properties: [
        { name: "customer_id", annotation: "str" },
        { name: "customerId", annotation: "str" }
      ]
    }
  ]);
  ...
  assert(aliasCamelOnly.firstLevel.includes("${alias.customer_id}"));
  assert(aliasCamelOnly.firstLevel.includes("${alias.customerId}"));
  ...
  assert(aliasSnakeOnly.firstLevel.includes("${alias.customer_id}"));
  assert(aliasSnakeOnly.firstLevel.includes("${alias.customerId}"));
}
...
function runRenderedArrowIndentHtmlTransformTests() {
  const inputHtml = [
    ...
    "[[RDP_INDENT_2]]-&gt; discounted amount = 12.00 EUR<br>",
    "[[RDP_INDENT_2]]continued explanation line<br>",
    "[[RDP_INDENT_2]]-&gt; payable amount = 108.00 EUR</li>",
    "</ul>",
  ...
  assert(transformedHtml.includes('class="robot-arrow-body">discounted amount = 12.00 EUR</span>'));
...
Case Arrow Continuation
    ...
    #>> - Bullet first line
    #>>   Bullet second line
    #> - Versandsperre setzen, damit die Bestellung nicht verarbeitet wird,
    #>   und somit noch im Warenkorb sichtbar ist.
    #> - Explicit next bullet
`);
  const parser = new extensionTestApi.RobotDocumentationService();
  const parsed = parser.parse(document);
  const block = parsed.blocks[0];
  ...
  assert.match(
    ...
    /-><span class="doc-target-marker" data-doc-target-index="0"><\/span> Arrow first line Arrow second line/
  );
  assert.doesNotMatch(bodyRenderData.markdown, /Arrow first line\s*\n/);
  assert.match(bodyRenderData.markdown, /Explicit next arrow/);
  assert.match(bodyRenderData.markdown, /Bullet first line Bullet second line/);
  assert.doesNotMatch(bodyRenderData.markdown, /Bullet first line\s*\n/);
  assert.match(
    bodyRenderData.markdown,
    /Versandsperre setzen, damit die Bestellung nicht verarbeitet wird, und somit noch im Warenkorb sichtbar ist\./
  );
  assert.doesNotMatch(bodyRenderData.markdown, /verarbeitet wird,\s*\n/);

  const listTargets = bodyRenderData.targets.filter((target) => target.kind === "list-item");
  assert.strictEqual(listTargets.length, 3, "expected each explicit bullet to keep one source target");
  assert(listTargets.some((target) => String(target.label).includes("line 6")));
  assert(listTargets.some((target) => String(target.label).includes("line 8")));
  assert(listTargets.some((target) => String(target.label).includes("line 10")));

  const arrowTargets = bodyRenderData.targets.filter((target) => target.kind === "arrow-line");
  assert.strictEqual(arrowTargets.length, 2, "expected each explicit arrow to keep one source target");
  assert(arrowTargets.some((target) => String(target.label).includes("line 3")));
  assert(arrowTargets.some((target) => String(target.label).includes("line 5")));

  const renderedHtml = await extensionTestApi.renderDocumentationBlockHtml(document.uri.toString(), block);
  assert.strictEqual(
    (renderedHtml.match(/class="robot-render-line robot-arrow-line"/g) || []).length,
    2,
    "expected only the two explicit arrows to use arrow-line rendering"
  );
  assert(!renderedHtml.includes("robot-arrow-marker-placeholder"));
  assert(renderedHtml.includes("Arrow first line Arrow second line"));
  assert(renderedHtml.includes("Bullet first line Bullet second line"));
  assert(
...
async function runStructuredDocumentationContinuationRenderingTests() {
  const document = createMockRobotDocument(`
*** Test Cases ***
Case Structured Continuation
    [Documentation]
    ...    ## Szenario
    ...    - Laufende Bestellung mit einer manuell erfassten Gutschrift (Beleg
    ...        GUTSCHRIFT erfasst -> Betrag negativ, grund=Kulanz) ohne Rücksendung.
    ...        -> Deckt DEMO-1001 Schritt 2, Schritt 5 und
    ...           Schritt 13 ab.
    ...    - Explicit next bullet
...
`);
  ...
  assert.match(
    bodyRenderData.markdown,
    /Beleg GUTSCHRIFT erfasst -> Betrag negativ, grund=Kulanz\) ohne Rücksendung\./
  );
  assert.doesNotMatch(bodyRenderData.markdown, /Beleg\s*\n/);
  assert.match(bodyRenderData.markdown, /-> Deckt DEMO-1001 Schritt 2, Schritt 5 und Schritt 13 ab\./);
  assert.doesNotMatch(bodyRenderData.markdown, /Schritt 5 und\s*\n/);
  assert.match(bodyRenderData.markdown, /Explicit next bullet with its own continuation\./);

  const listTargets = bodyRenderData.targets.filter((target) => target.kind === "list-item");
  assert.strictEqual(listTargets.length, 2, "expected each explicit structured bullet to keep one source target");
  assert(listTargets.some((target) => String(target.label).includes("line 5")));
  assert(listTargets.some((target) => String(target.label).includes("line 9")));
  ...
  assert(renderedHtml.includes("Deckt DEMO-1001 Schritt 2, Schritt 5 und Schritt 13 ab."));

  const exportedMarkdown = extensionTestApi.buildDocumentationExportMarkdown(document.uri.toString(), block);
  assert.match(exportedMarkdown, /Beleg GUTSCHRIFT erfasst -> Betrag negativ/);
  assert.match(exportedMarkdown, /-> Deckt DEMO-1001 Schritt 2, Schritt 5 und Schritt 13 ab\./);
}

async function runDocumentationIntentionalLineBreakTests() {
  const inlineDocument = createMockRobotDocument(`
*** Test Cases ***
Case Inline Intentional Breaks
    #> - Bullet first line.<br>
    #>   Bullet intentional second line.
    #>> -> Arrow first line.<br>
    #>>    Arrow intentional second line.
`);
  const parser = new extensionTestApi.RobotDocumentationService();
  const inlineBlock = parser.parse(inlineDocument).blocks[0];
  const inlineHtml = await extensionTestApi.renderDocumentationBlockHtml(
    inlineDocument.uri.toString(),
    inlineBlock
  );

  assert(inlineHtml.includes("Bullet first line.<br> Bullet intentional second line."));
  assert.match(
    inlineHtml,
    /class="robot-arrow-body">[\s\S]*Arrow first line\.<br> Arrow intentional second line\.\s*<\/span>/
  );
  assert(!inlineHtml.includes("&lt;br&gt;"));

  const structuredDocument = createMockRobotDocument(`
*** Test Cases ***
Case Structured Intentional Break
    [Documentation]
    ...    - Scenario summary.
    ...        -> Structured arrow first line.<br>
    ...           Structured arrow intentional second line.
`);
  const structuredBlock = parser.parse(structuredDocument).blocks[0];
  const structuredHtml = await extensionTestApi.renderDocumentationBlockHtml(
    structuredDocument.uri.toString(),
    structuredBlock
  );

  assert.match(
    structuredHtml,
    /class="robot-arrow-body">Structured arrow first line\.<br> Structured arrow intentional second line\.\s*<\/span>/
  );
  const exportedMarkdown = extensionTestApi.buildDocumentationExportMarkdown(
    structuredDocument.uri.toString(),
    structuredBlock
  );
  assert.match(exportedMarkdown, /Structured arrow first line\.<br> Structured arrow intentional second line\./);
}

async function runDocumentationPlainTextContinuationTests() {
  assert.deepStrictEqual(
    extensionTestApi.normalizeDocumentationParagraphLines([
      "Plain prose first line,",
      "plain prose second line.",
      "```robotframework",
      "Log    content that must stay fenced",
      "```"
    ]),
    [
      "Plain prose first line, plain prose second line.",
      "```robotframework",
      "Log    content that must stay fenced",
      "```"
    ]
  ...
  const inlineDocument = createMockRobotDocument(`
*** Test Cases ***
Case Inline Plain Continuations
    #> Plain inline first line,
    #> plain inline second line.
    #>
    #> Plain inline explicit break.<br>
    #> plain inline intentional next line.
`);
  const parser = new extensionTestApi.RobotDocumentationService();
  const inlineBlock = parser.parse(inlineDocument).blocks[0];
  const inlineRenderData = extensionTestApi.buildDocumentationBodyRenderData(
    inlineDocument.uri.toString(),
    inlineBlock
  );
  assert(inlineRenderData.markdown.includes("Plain inline first line, plain inline second line."));
  assert.doesNotMatch(inlineRenderData.markdown, /Plain inline first line,\s*\n/);
  assert(
    inlineRenderData.markdown.includes(
      "Plain inline explicit break.<br> plain inline intentional next line."
    )
  );
  ...
  assert(inlineHtml.includes("Plain inline first line, plain inline second line."));
  assert(inlineHtml.includes("Plain inline explicit break.<br> plain inline intentional next line."));

  const structuredDocument = createMockRobotDocument(`
*** Test Cases ***
Case Structured Plain Continuations
    [Documentation]
    ...    Plain structured first line,
    ...    plain structured second line.
    ...
    ...    Plain structured explicit break.<br>
    ...    plain structured intentional next line.
`);
  const structuredBlock = parser.parse(structuredDocument).blocks[0];
  const structuredRenderData = extensionTestApi.buildDocumentationBodyRenderData(
    structuredDocument.uri.toString(),
    structuredBlock
  );
  assert(
    structuredRenderData.markdown.includes(
      "Plain structured first line, plain structured second line."
    )
  );
  assert.doesNotMatch(structuredRenderData.markdown, /Plain structured first line,\s*\n/);
  assert(
    structuredRenderData.markdown.includes(
      "Plain structured explicit break.<br> plain structured intentional next line."
    )
  );
  ...
  assert(structuredHtml.includes("Plain structured first line, plain structured second line."));
  assert(
    structuredHtml.includes(
      "Plain structured explicit break.<br> plain structured intentional next line."
    )
  );
}
...
async function runLargeFixtureRenderTargetTests() {
  const fixtureExpectations = [
    ...
    {
      fixtureName: "folding-regression-refund.robot",
      expectedTargets: [
    ...
    {
      fixtureName: "documentation-arrow-indent-shipping.robot",
      expectedTargets: [
        { line: 6, kind: "heading", labelFragment: "line 7" },
        { line: 7, kind: "list-item", labelFragment: "line 8" },
        { line: 8, kind: "list-item", labelFragment: "line 9" },
...
Case Documentation Variables
    \${datumBestellung}=    Set Variable    17.03.2025
    \${datumVersand}=    Set Variable    \${datumBestellung}
    \${zweiterAlias}=    Set Variable    \${datumVersand}
    VAR    \${datumNachlieferung}    24.07.2026
    \${sameValue}=    Set Variable    A_B
    \${sameValue}=    Set Variable    A_B
    \${ambiguousDate}=    Set Variable    2025-01-01
    \${ambiguousDate}=    Set Variable    2025-02-01
    #> - Artikel neu: 2 Artikel ab \${datumBestellung}, 1 Artikel ab \${datumNachlieferung}, 0 Artikel ab 23.11.2027.
    #>> -> Lieferdaten ab \${datumVersand} im XML an das Versandsystem.
...
`);
  ...
  assert(bodyHtml.includes("2 Artikel ab 17.03.2025, 1 Artikel ab 24.07.2026"));
  assert(bodyHtml.includes("Lieferdaten ab 17.03.2025 im XML an das Versandsystem."));
  assert(bodyHtml.includes("Zweiter Alias ab 17.03.2025 bleibt konkret."));
  assert(!bodyHtml.includes("${datumNachlieferung}"));
  assert(!bodyHtml.includes("${datumVersand}"));
  ...
  assert(bodyHtml.includes("fenced ${datumBestellung}"));
}
...
Case Documentation Colors
    #> ## Flow
    #> - Prüfen, ob <question>die Preisregel noch offen ist</question> und <color value="red">rot markiert</color> wird.
    #>> -> Ergebnis ist <error>noch fehlerhaft</error>, siehe <color value="#0f766e">neue Rückfrage</color> und <pink>pink markiert</pink>.
...
`);
  ...
  assert(markdown.includes("<question>die Preisregel noch offen ist</question>"));
  assert(markdown.includes('<color value="red">rot markiert</color>'));
  assert(markdown.includes("<error>noch fehlerhaft</error>"));
  assert(markdown.includes('<color value="#0f766e">neue Rückfrage</color>'));
...
Case Conditional Keyword Branch
    IF    \${checkPdf}
        \${pdfContent}=    Demo PDF Read By TrackingId
        ...    trackingId=\${trackingId}
    ELSE
        \${pdfContent}=    Set Variable    \${None}
    END
    Keyword Under Test    pdfContent=\${pdfContent}
`);
  ...
  const keywordBranchCurrentValue = extensionTestApi.resolveNamedArgumentCurrentValueFromSetVariable(
    {
      argumentValue: "${pdfContent}",
      valueStart: "    Keyword Under Test    pdfContent=".length
    },
    keywordBranchParsed,
    8
  );
  ...
  assert.deepStrictEqual(
    ...
    ["Return from Demo PDF Read By TrackingId", "${None}"]
  );
  ...
  assert.match(keywordBranchMarkdown, /Return from Demo PDF Read By TrackingId/);
...
`);
  ...
  const prefixedArgumentCurrentValue = extensionTestApi.resolveNamedArgumentCurrentValueFromSetVariable(
    {
      argumentValue: "ErsterTagDerNächstenWoche ${typedDate}",
      valueStart: 28
    },
    parsed,
    6,
    undefined,
    28 + "ErsterTagDerNächstenWoche ".length + 3
  );
  ...
  assert.strictEqual(prefixedArgumentFallback.value, "ErsterTagDerNächstenWoche ${typedDate}");
...
Case Alias Resolution
    ...
    IF    \${condition}
        \${branchValue}=    Set Variable    \${datumBestellung}
    ELSE
        \${branchValue}=    Set Variable    23.04.2025
    END
    Demo Keyword    datumVersand=\${datumVersand}
...
`);
  ...
  const aliasHover = extensionTestApi.createVariableValueHover(
    ...
    {
      line: 12,
      character: document.lineAt(12).text.indexOf("${datumVersand}") + 3
    }
  );
  assert(aliasHover, "expected hover for alias variable");
  assert.match(aliasHover.contents[0].value, /`17\.03\.2025`/);
  assert.match(aliasHover.contents[0].value, /\*\*Source:\*\* `Set Variable` at line 4/);
  assert.doesNotMatch(aliasHover.contents[0].value, /\$\{datumBestellung\}/);

  const aliasArgumentValue = extensionTestApi.resolveNamedArgumentCurrentValueFromSetVariable(
    {
      argumentValue: "${datumVersand}",
      valueStart: document.lineAt(12).text.indexOf("${datumVersand}")
    },
    parsed,
    12
  );
  assert.strictEqual(aliasArgumentValue.value, "17.03.2025");
  ...
  assert.strictEqual(twoHopArgumentValue.value, "17.03.2025");
...
Case Embedded Keyword Return
    ${"${OrderPrefix}"}=    Set Variable    PREFIX
    ${"${OD}"}=    Demo Order Data Write To Mock Database
    Demo Keyword    value=prefix${"${OrderPrefix}"}${"${OD.OrderNr}"}postfix
`);
  const parser = new extensionTestApi.RobotDocumentationService();
  const parsed = parser.parse(document);
  const argumentValue = `prefix${"${OrderPrefix}"}${"${OD.OrderNr}"}postfix`;
  ...
  const hoveredReturnVariablePreview = await extensionTestApi.resolveEnumValuePreviewFromContext(
    ...
    },
    {
      parsed,
      referenceLine: 4,
      hoverCharacter: "prefix${OrderPrefix}".length + 3
    }
  );
  assert(hoveredReturnVariablePreview, "expected argument preview for embedded return variable");
  assert.strictEqual(hoveredReturnVariablePreview.currentValue, "${OD.OrderNr}");
  ...
  assert.match(hoveredReturnVariableMarkdown, /`\$\{OD\.OrderNr\}`/);
  assert.match(
    hoveredReturnVariableMarkdown,
    /from `prefix\$\{OrderPrefix\}\$\{OD\.OrderNr\}postfix`/
  );
...
Case HeadingThenPlainMarker
    #> ### Prepare test data
    Log    prep one
    #> 1. Write order data to the mock database
    Log    prep two
    #> ### Submit the order and start batch processing
    Log    step two
Case PlainClassic
...
Case MarkerDepthHierarchyNoGap
    #> - first level
    \${CaseId}=    Demo Case Id For Order Number Resolve
    ...    orderNumber=\${OD.OrderNr}
    #>> - child level
    Demo Order Log Entry Check - DatabaseCall
    ...    caseId=\${CaseId}
    #> - next first level
    Demo Delivery Status Check - DatabaseCall
...
Case HeadingWithPlainPeersAndNestedChild
    ...
    #> - next first level
    Log    next body
    #> - Check the generated export order file
    Log    last body
Case NoFold
    # comment only
    Log    noop
...
function runUmlautDecoratorNameTests() {
  const manifest = JSON.parse(fs.readFileSync(path.join(__dirname, "..", "package.json"), "utf8"));
  assert.deepStrictEqual(
    manifest.contributes.configuration.properties["robotCompanion.umlautDecoratorNames"].default,
    ["convert_umlaut_kwargs"],
    "the shipped default must stay the name that was hardcoded before it became a setting"
  );
  assert.deepStrictEqual(
    manifest.contributes.configuration.properties["robotCompanion.umlautExcludeSymbols"].default,
    ["_exclude_umlaut_kwargs"],
    "same for the exclusion-list symbol"
  );

  const api = loadExtensionModule().__test__;
  const { resetUmlautNamesCache } = api;
  const sourceFor = (decorator, exclude) => `
${exclude} = [
    "waehrung",
    "einheit",
]

def ${decorator}(exclude=None):
    pass
`;

  // default names still parse, through the real indexing entry point
  resetUmlautNamesCache();
  const byDefault = api.parseConvertUmlautDecoratorConfigFromPythonSource(
    sourceFor("convert_umlaut_kwargs", "_exclude_umlaut_kwargs"), "/w/lib/u.py");
  assert(byDefault, "the shipped default names must still be recognised");
  assert.deepStrictEqual(byDefault.defaultExcludeKeys, ["waehrung", "einheit"]);

  // a project naming them differently is recognised only once configured
  const customSource = sourceFor("umlaut_args", "_skip_umlaut");
  resetUmlautNamesCache();
  assert.strictEqual(
    api.parseConvertUmlautDecoratorConfigFromPythonSource(customSource, "/w/lib/u.py"),
    undefined,
    "custom names must not be recognised while the default is in force"
  );

  testConfigOverrides["umlautDecoratorNames"] = ["umlaut_args"];
  testConfigOverrides["umlautExcludeSymbols"] = ["_skip_umlaut"];
  resetUmlautNamesCache();
  const configured = api.parseConvertUmlautDecoratorConfigFromPythonSource(customSource, "/w/lib/u.py");
  assert(configured, "a configured decorator/exclude pair must be recognised end to end");
  assert.deepStrictEqual(configured.defaultExcludeKeys, ["waehrung", "einheit"]);
  assert.strictEqual(
    api.parseConvertUmlautDecoratorConfigFromPythonSource(
      sourceFor("convert_umlaut_kwargs", "_exclude_umlaut_kwargs"), "/w/lib/u.py"),
    undefined,
    "an explicit list replaces the default rather than extending it"
  );

  // regex metacharacters in a configured name must be escaped, not interpolated
  testConfigOverrides["umlautDecoratorNames"] = ["odd.name+v2"];
  testConfigOverrides["umlautExcludeSymbols"] = ["_skip_umlaut"];
  resetUmlautNamesCache();
  assert(
    api.parseConvertUmlautDecoratorConfigFromPythonSource(
      sourceFor("odd.name+v2", "_skip_umlaut"), "/w/lib/u.py"),
    "names must be escaped before they reach a RegExp"
  );

  // an empty list must not disable a working feature
  testConfigOverrides["umlautDecoratorNames"] = [];
  testConfigOverrides["umlautExcludeSymbols"] = [];
  resetUmlautNamesCache();
  assert(
    api.parseConvertUmlautDecoratorConfigFromPythonSource(
      sourceFor("convert_umlaut_kwargs", "_exclude_umlaut_kwargs"), "/w/lib/u.py"),
    "an empty list falls back to the shipped default"
  );

  delete testConfigOverrides["umlautDecoratorNames"];
  delete testConfigOverrides["umlautExcludeSymbols"];
  resetUmlautNamesCache();
}

function runCamelCaseAliasBaseNameTests() {
  const manifest = JSON.parse(fs.readFileSync(path.join(__dirname, "..", "package.json"), "utf8"));
  assert.deepStrictEqual(
    manifest.contributes.configuration.properties["robotCompanion.camelCaseAliasBaseNames"].default,
    ["camelcase"],
    "the shipped default must stay the rule that was hardcoded before it became a setting"
  );

  const { isCamelCaseAliasBaseName, resetCamelCaseAliasBaseNamesCache } = loadExtensionModule().__test__;

  // default: exactly the previous behaviour, normalisation included
  resetCamelCaseAliasBaseNamesCache();
  for (const marked of ["CamelCaseBase", "CamelCaseModel", "Camel_Case_Base", "MyCamelCaseConverter"]) {
    assert.ok(isCamelCaseAliasBaseName(marked), `${marked} should mark camelCase alias support`);
  }
  for (const plain of ["BaseModel", "SnakeToCamelMixin", "object", ""]) {
    assert.ok(!isCamelCaseAliasBaseName(plain), `${plain} must not mark camelCase alias support`);
  }

  // a project whose converter base is named something else can say so
  testConfigOverrides["camelCaseAliasBaseNames"] = ["ApiModel"];
  resetCamelCaseAliasBaseNamesCache();
  assert.ok(isCamelCaseAliasBaseName("ApiModel"), "a configured marker should enable aliases");
  assert.ok(
    !isCamelCaseAliasBaseName("CamelCaseBase"),
    "an explicit list replaces the default rather than extending it"
  );

  // an empty list means empty, which is why the default is a real value
  testConfigOverrides["camelCaseAliasBaseNames"] = [];
  resetCamelCaseAliasBaseNamesCache();
  assert.ok(!isCamelCaseAliasBaseName("CamelCaseBase"), "an empty list disables aliasing entirely");

  // a malformed value must not disable a working feature
  testConfigOverrides["camelCaseAliasBaseNames"] = "camelcase";
  resetCamelCaseAliasBaseNamesCache();
  assert.ok(isCamelCaseAliasBaseName("CamelCaseBase"), "a non-array value falls back to the default");

  delete testConfigOverrides["camelCaseAliasBaseNames"];
  resetCamelCaseAliasBaseNamesCache();
}
...
`;
  const parsedConfig = extensionTestApi.parseConvertUmlautDecoratorConfigFromPythonSource(
    decorationSource,
    "/workspace/lib/umlaut_config.py"
  );
  assert(parsedConfig, "Expected convert_umlaut decorator config to be parsed from source.");
  assert.deepStrictEqual(parsedConfig.defaultExcludeKeys, ["waehrung", "einheit", "value", "request"]);

  const keywordSource = `
from robotlib.decorators import convert_umlaut_kwargs, keyword

@keyword("Demo Keyword")
@convert_umlaut_kwargs()
def demo_keyword(
    hinweisRuecksendung: str,
    waehrung: str,
    UebersichtInfo: str,
    value: str,
):
    """
    Args:
        hinweisRuecksendung (str): Beschreibt die Rücksendung.
        waehrung (str): Sollte wegen der Exclusion ascii bleiben.
        UebersichtInfo (str): Uppercase Umlaut replacement.
...
`;
  const parsedDefinitions = extensionTestApi.parseKeywordEnumHintsFromPythonSource(
    keywordSource,
    "/workspace/Demo/Keywords/demo_keyword.py"
  );
  ...
  assert.deepStrictEqual(
    ...
    [
      ["hinweisRücksendung", "str"],
      ["waehrung", "str"],
      ["ÜbersichtInfo", "str"],
      ["value", "str"]
    ]
  );
  assert.match(finalized.normalizedDocstring, /`hinweisRücksendung`/);
  assert.match(finalized.normalizedDocstring, /`waehrung`/);
  assert.match(finalized.normalizedDocstring, /`ÜbersichtInfo`/);
  ...
  assert.match(previewMarkdown, /hinweisRücksendung/);
  assert.match(previewMarkdown, /ÜbersichtInfo/);
  ...
  const umlautInsertPayload = insertPayloads.find(
    (payload) => payload.argumentName === "hinweisRücksendung"
  );
...
@convert_umlaut_kwargs(exclude=("hinweisRuecksendung",))
def custom_keyword(hinweisRuecksendung: str):
    """
    Args:
        hinweisRuecksendung (str): Bleibt ascii.
    """
    return None
`;
  const customDefinition = extensionTestApi.parseKeywordEnumHintsFromPythonSource(
    customExcludeSource,
    "/workspace/Demo/Keywords/custom_keyword.py"
  )[0];
  const finalizedCustom = extensionTestApi.finalizePythonKeywordDefinitionForIndex(customDefinition, {
    defaultExcludeKeys: parsedConfig.defaultExcludeKeys
  });
  assert.deepStrictEqual([...finalizedCustom.parameters.keys()], ["hinweisRuecksendung"]);
  assert.match(finalizedCustom.normalizedDocstring, /`hinweisRuecksendung`/);
...
def dynamic_keyword(waehrung: str, hinweisRuecksendung: str):
    """
    Args:
        waehrung (str): Default exclusion still applies.
        hinweisRuecksendung (str): Regular conversion should still happen.
    """
    return None
`;
  const fallbackDefinition = extensionTestApi.parseKeywordEnumHintsFromPythonSource(
    fallbackExcludeSource,
    "/workspace/Demo/Keywords/dynamic_keyword.py"
  )[0];
...
Case Umlauts Still Match
    Demo Keyword    hinweisRuecksendung=ja
`);
  const existingPlan = extensionTestApi.buildKeywordArgumentInsertPlan(asciiDocument, {
    keywordLine: 2,
    argumentName: "hinweisRücksendung",
    documentedArgumentNames: ["hinweisRücksendung"]
  });
  assert(existingPlan);
  assert.strictEqual(existingPlan.kind, "existing");
  assert.strictEqual(existingPlan.existingTarget.argumentName, "hinweisRuecksendung");
...
`);
  ...
  assert.strictEqual(insertPlan.insertLineText, "    ...    hinweisRücksendung=");
}
...
async function main() {
  ...
  await runStructuredDocumentationContinuationRenderingTests();
  await runDocumentationIntentionalLineBreakTests();
  await runDocumentationPlainTextContinuationTests();
  ...
  runCamelCaseAliasBaseNameTests();
  runUmlautDecoratorNameTests();

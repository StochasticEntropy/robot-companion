...
const FIXTURE_SCENARIOS = [
  ...
  },
  {
    ...
    label: "large refund testcase owner",
    ownerLine: 13,
...
const MIXED_DOCUMENTATION_PREVIEW_SCENARIOS = [
  {
    fixtureName: "documentation-plain-continuation.robot",
    expectedTargetLinesInOrder: [2, 5],
    previewSourceJumpLines: [2, 5],
    expectedFlowingHtml: [
      "Plain first line, plain second line.",
      "Plain intentional first line.<br> plain intentional second line."
    ]
  },
  {
    fixtureName: "documentation-intentional-break.robot",
    expectedTargetLinesInOrder: [2, 4],
    previewSourceJumpLines: [2, 4],
    expectedArrowIndentWidths: [2],
    expectedIntentionalBreakHtml: [
      "Bullet first line.<br> Bullet intentional second line.",
      "Arrow first line.<br> Arrow intentional second line."
    ]
  },
  {
...
suite("Robot Companion documentation folding UI", function () {
  ...
  for (const scenario of FIXTURE_SCENARIOS) {
    suite(`${scenario.label} fixture`, () => {
      ...
      if (scenario.fixtureName === "folding-regression-refund.robot") {
  ...
  suite("mixed documentation and inline preview targets", () => {
    for (const scenario of MIXED_DOCUMENTATION_PREVIEW_SCENARIOS) {
      test(`${scenario.fixtureName} keeps top documentation targets aligned with later inline comments`, async () => {
        ...
        for (const expectedHtml of Array.isArray(scenario.expectedIntentionalBreakHtml)
          ? scenario.expectedIntentionalBreakHtml
          : []) {
          assert(
            renderedHtml.includes(expectedHtml),
            `${scenario.fixtureName} should preserve the intentional line break in ${expectedHtml}`
          );
        }

        for (const expectedHtml of Array.isArray(scenario.expectedFlowingHtml)
          ? scenario.expectedFlowingHtml
          : []) {
          assert(
            renderedHtml.includes(expectedHtml),
            `${scenario.fixtureName} should render flowing plain documentation as ${expectedHtml}`
          );
        }
